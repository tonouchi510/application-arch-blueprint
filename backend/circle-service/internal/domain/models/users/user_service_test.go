...
func (s *UserServiceTestSuite) TestExists() {
	...
	t.Run("Success/同じ名前のユーザが存在する場合、trueを返す", func(t *testing.T) {
		repo.EXPECT().FindByName(s.ctx, s.userName).Return(s.user, nil)
		exist, err := domainService.Exists(s.ctx, s.user)
		require.NoError(t, err)
		require.True(t, exist)
	})

	t.Run("Success/同じ名前のユーザが存在しない場合、falseを返す", func(t *testing.T) {
		repo.EXPECT().FindByName(s.ctx, s.userName).Return(nil, fmt.Errorf("sql: no rows in result set"))
		exist, err := domainService.Exists(s.ctx, s.user)
		require.NoError(t, err)
	...
	t.Run("Error/FindByNameでエラーが発生した場合、エラーを返す", func(t *testing.T) {
		repo.EXPECT().FindByName(s.ctx, s.userName).Return(nil, fmt.Errorf("some error"))
		exist, err := domainService.Exists(s.ctx, s.user)
		require.Error(t, err)
		require.False(t, exist)
	})
